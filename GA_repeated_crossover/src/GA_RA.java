...
//		ne=worst(population,100,ETC);
//		for(i=0;i<2;i++){
////			System.out.println("ssssssss "+ne[i]);
//		}
...
//							Entity2 now=Mutation(arr,mut,mut.getFit(arr));
					...
					population[id].setEnt(s5);
					fitres[id]=Fitness1(s5,ETC);
//					System.out.println("FITafter MUTATION --"+Fitness1(population[id].getEnt(),ETC));
					
//							population[id]=now;
							//if(Fitness1(population[id].getEnt(),ETC)>get){
								//population[id]=mut1;
							//}
							//System.out.println("ID FIT --"+Fitness1(population[id].getEnt(),ETC));
...
//			sort(population,100,ETC);
			...
			int id = 0;
//			double mutPercent = generator1.nextInt(50) ;
			id=minindex(fitres,100,ETC);
//			 id=generator.nextInt(100);
			
			 Entity2 mut=population[id];
			 Entity2 mut1=population[id];
			 double get=Fitness1(population[id].getEnt(),ETC);
//			System.out.println("ID MUTATION --"+id);
//			System.out.println("FITB7 MUTATION --"+get);
...
//					Entity2 now=Mutation(arr,mut,mut.getFit(arr));
			...
			String s5=IntelligentMutation(ETC,s7,fitres,id);
			population[id].setEnt(s5);
//			System.out.println("FITafter7 MUTATION --"+Fitness1(population[id].getEnt(),ETC));
...
//					population[id]=now;
					...
					System.out.println("ID FIT --"+Fitness1(population[id].getEnt(),ETC));
//	sort(population,150,arr);
//	for(i=0;i<100;i++){
//		double ft=Fitness1(population[i].getEnt(),ETC);
////		System.out.println("Fitnessafter  :"+i+"  "+ft);
//	}
//	population[9]=population[0];
//	population[98]=population[1];
//	if(sample1>sample2){
//	if(p1>=sample)
//	populationnew[count]=offspring[0];
//	count++;
//	populationnew[count]=offspring[1];
//	count++;
...
public static int[] worst (Entity2 arr[], int length,double ETC[][]){
 	...
 	Entity2 temp[]=new Entity2[2];
 	Entity2 temp1=new Entity2();
 	Entity2 temp2=new Entity2();
// 	for(int i=0;i<2;i++){
// 		Entiity2 temp[0]=new Entity2();
// 	}
 	double id0=Fitness1(arr[0].getEnt(),ETC);
 	double id1=Fitness1(arr[1].getEnt(),ETC);
 	temp1.setFitProb(0);
 	temp2.setFitProb(0);
 	if(id0>id1){
 	temp1.setEnt(arr[0].getEnt());
 	index[0]=0;
 	index[1]=1;
 	temp2.setEnt(arr[1].getEnt());
 	}
 	else{
 		temp1.setEnt(arr[1].getEnt());
 	 	temp2.setEnt(arr[0].getEnt());
 	 	index[0]=0;
 	 	index[1]=1;
 	}
	...
	for(int m=1;m<100;m++){
		double id= Fitness1(arr[m].getEnt(),ETC) ;
		if(id>id0){
			String s1=temp1.getEnt();
		temp1.setEnt(arr[m].getEnt());
		temp2.setEnt(s1);
		id0=id;
		index[1]=index[0];
		index[0]=m;
